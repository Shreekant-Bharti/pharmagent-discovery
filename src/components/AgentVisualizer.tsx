...
import { Scroll, FlaskConical, BarChart3, CheckCircle2, Loader2, Cpu, Zap, ChevronRight } from 'lucide-react';
...
const AgentVisualizer = ({ isActive, masterStatus, agents, logs }: AgentVisualizerProps) => {
  const [logsExpanded, setLogsExpanded] = useState(true);
  const [selectedAgent, setSelectedAgent] = useState<AgentTask | null>(null);
  ...
  return (
    <>
                    ...
                    {agents.map((agent, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          onClick={() => agent.status === 'success' && setSelectedAgent(agent)}
                          className={`agent-card transition-all ${
      ...
      <AgentCardModal
        ...
        rawData={selectedAgent?.rawJson || ''}
        abstract={selectedAgent?.abstract}
      />
    </>
  );
};
